...
#------------------------------
df2 = pd.read_excel('other/Adhoc_QB_CH_Payer_Entities_31_10_2023.xlsx',sheet_name='Trigger_Words')
...
#-----------------------------------------------------
# Create 'Payer_Params_Dict' with Preprocessing'
#-----------------------------------------------------
df_overall = pd.read_excel('other/Adhoc_QB_CH_Payer_Entities_31_10_2023.xlsx',sheet_name='Params_Sheet')
column_names= df_overall.columns                                               # Get all columns names

all_payer_params_dict={}                                      
for column in column_names:
    ...
    all_payer_params_dict[column]= column_entity_list                           # insert complete column list in dict 
...
def payer_extraction(f_text):
    ...
    #--------------------------------------------------
    payer_params_dict={}                                                   # 'final dict' to save result
    for field_name_key,param_field_list_value in all_payer_params_dict.items():
        if field_name_key in ['Coverage ID Payer','Coverage type Payer','Coverage Display Name Payer','Start Date Payer','End Date Payer',
                              'Patient Name Payer','Patient First Name Payer','Patient Last Name Payer','Subscriber ID Payer','Display Name Payer',
                              'Source Payer','Insurance Type Code Payer']:
        ...
        if extracted_param_list:                                           # check: if list is not empty
            try:
                payer_params_dict[field_name_key].extend(extracted_param_list)
            except:
                payer_params_dict[field_name_key]= extracted_param_list

                   
    return [payer_params_dict,f_text]
...
'''
text= "Show me all the Payer data  whose last name is JOhn with code id 2231 "
clean_text= text_clean(text)
result= payer_extraction(clean_text)
print('\n',result[0])
print('\n',result[1])
'''
