...
"""
    Module: QB project
    Extract all params date, specific for 'Claims'
...
#==============================================================================
# function: to extract param field from dictionary (like ckd -> Problem),for claim fileds return 'Claims'
def paramfield(f_param,f_dict):
    for f_key,f_value in f_dict.items():
        if f_param in f_value:
            if f_key in ['Risk Score','Radiology Visit','Readmission Count','ER Visit','Inpatient Spend',
                         'Hospital outpatient Spend','Pharmacy Spend','Professional outpatient Spend',
                         'SNF Spend','Home Health Spend','Hospice Spend','DME Spend','Total Spend']:
                return 'Claim'
        
        elif f_param in f_value:
            return f_key
...
text= "snf cost is above 100 and  total cost is below 200 in 2020"
text_clean_result= text_clean(text)
initial_extracted_params= claims_extraction(text_clean_result)
optimize_params= param_optimize(text_clean_result,initial_extracted_params[0])

result= param_date_module2(optimize_params[1],optimize_params[0])
print('\nFinal Dic=\n',result)
